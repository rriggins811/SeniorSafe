...
-- households: owners (no invited_by, not test). self_setup: the owner is the
-- senior (the legacy one-sided accounts all look like this). parent_joined_48h:
-- a separate senior profile joined within 48 hours of the owner. checked_in: any
...
drop view if exists public.funnel_totals;
drop view if exists public.funnel_dashboard;
create view public.funnel_dashboard as
with owners as (
...
select
  ...
  count(*) filter (where o.is_senior)::int as self_setup,
  count(*) filter (where not coalesce(o.is_senior, false) and exists (
    select 1 from fam f
...
create view public.funnel_totals as
select sum(households)::int as households, sum(self_setup)::int as self_setup,
       sum(parent_joined_48h)::int as parent_joined_48h, sum(checked_in)::int as checked_in,
       sum(lock_taps)::int as lock_taps, sum(paid)::int as paid
