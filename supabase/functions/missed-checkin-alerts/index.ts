...
async function sendFailureAlertEmail(familyName: string, phone: string, seniorName: string, errorDetail: string): Promise<boolean> {
  ...
  try {
    const res = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({ from: `Hammock365 Alerts <${FROM_ADDRESS}>`, to: ['support@hammock365.com'], subject: 'ALERT: Missed check-in SMS failed', text: body }),
    })
...
serve(async (req) => {
  ...
  for (const senior of seniors as ProfileRow[]) {
    try {
      ...
      const message = `${seniorName} hasn't checked in today. Hammock365. Reply STOP to opt out`
