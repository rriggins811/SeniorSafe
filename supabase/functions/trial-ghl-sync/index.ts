...
// trial-ghl-sync (scheduled every 15 min via pg_cron). Wires Hammock365 app
...
serve(async (req: Request) => {
  ...
  // link or code also gets hammock-from-<code> (partner co-branding, Level 1;
  // the hammock- prefix keeps it clear of RSS partner tags, Ryan 9/12).
  ...
  for (const u of newTrials ?? []) {
    ...
    if (u.partner_code) tags.push(`hammock-from-${u.partner_code}`)
    try {
