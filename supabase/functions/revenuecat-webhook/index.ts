    ...
    case 'BILLING_ISSUE': {
      if (profile.phone) {
        const ok = await sendTwilioSMS(
          ...
          `SeniorSafe could not renew your subscription through ${platform === 'google' ? 'Google Play' : 'the App Store'}. Update your payment method there so the family texts keep going. Reply STOP to opt out`,
        )
