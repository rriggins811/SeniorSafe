  ...
  return (
    <div className="min-h-screen bg-[#FAF8F4] flex flex-col">
      <div className="bg-[#1B365D] px-5 pt-12 pb-5 flex-shrink-0">
        <div className="max-w-lg mx-auto">
          ...
          <div className="flex items-center gap-3">
            <div className="bg-white/15 rounded-xl p-2"><Icon size={22} color="#D4A843" strokeWidth={1.5} /></div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6 py-14 text-center gap-5 max-w-lg mx-auto w-full">
        <div className="bg-[#1B365D] rounded-2xl p-5"><Lock size={40} color="#D4A843" strokeWidth={1.5} /></div>
        <div>
          <h2 className="text-[#1B365D] font-bold mb-2" style={{ fontSize: '22px' }}>{title} is on the paid plan</h2>
          <p className="text-[#6B645A] leading-relaxed" style={{ fontSize: '17px' }}>{description}</p>
        </div>
        {isSenior ? (
          <p className="text-[#1B365D] leading-relaxed" style={{ fontSize: '17px' }}>
          ...
          <>
            <button onClick={go} className="w-full max-w-xs py-4 rounded-xl bg-[#D4A843] text-[#1B365D] font-bold" style={{ fontSize: '18px' }}>
              See the paid plan
            </button>
            <p className="text-[#6B645A]" style={{ fontSize: '15px' }}>{priceLine()}</p>
          </>
        )}
        <button onClick={() => navigate(back)} className="text-[#1B365D] underline" style={{ fontSize: '16px' }}>Not now</button>
      </div>
    </div>
  )
}
