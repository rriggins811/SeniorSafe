import {
  ...
  ChevronRight, Phone, Heart, FolderLock, Settings, Sparkles, Home, Users,
} from 'lucide-react'
...
export default function FamilyHome({
  ...
  const late = preview
    ? !checkedIn
    : adminCheckInLoaded && !checkedIn && hour >= 10
  ...
  if (!adminCheckInLoaded) {
    ...
    status = {
      ...
      body: `${name} has not tapped I'm Okay.`,
      ...
      <div className="px-4 pt-5 pb-4 max-w-lg mx-auto flex flex-col gap-4">
          ...
          <div className="bg-orange-50 border-2 border-orange-300 rounded-2xl p-4 flex items-start gap-3">
            ...
            <button type="button" className="flex-1 text-left" onClick={onOpenFailedNotification}>
              ...
              <p className="text-orange-700 text-sm mt-0.5 leading-relaxed">
                We had trouble delivering a notification. Check phone numbers in settings.
              </p>
            </button>
        ...
        {trialDays !== null && trialDays <= 4 && trialDays > 0 && !trialBannerDismissed && (
          <div className="bg-amber-50 border-2 border-amber-300 rounded-2xl p-4 flex items-start gap-3">
            ...
            <button onClick={onDismissTrial} className="text-amber-400 text-lg leading-none">&times;</button>
          </div>
        )}
        ...
        {smsToast && (
          <div className="bg-blue-50 border border-blue-200 rounded-2xl p-3 flex items-center gap-3">
            ...
            <button onClick={onDismissToast} className="text-blue-400 text-sm">&times;</button>
          </div>
        )}
        ...
        <section className={`rounded-3xl border-2 p-5 ${status.wrap}`}>
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              {adminCheckIn?.note && (
                <p className="text-[#1B365D] text-sm mt-2 italic">&ldquo;{adminCheckIn.note}&rdquo;</p>
              )}
            </div>
          </div>

          {!checkedIn && adminCheckInLoaded && (
            <div className="mt-4">
              {premium ? (
                nudgeCount >= 2 ? (
                  <p className="text-sm leading-relaxed text-[#7A2E28]">
                  ...
                  <>
                    ...
                    {nudgeWarning && (
                      <p className="text-sm mt-2 leading-relaxed text-[#6B645A]">{nudgeWarning}</p>
                    )}
                  </>
                )
              ) : (
                <button
                  onClick={() => onNavigate('/upgrade')}
                  className="w-full py-3 rounded-xl bg-white border border-[#E7E2D8] text-[#1B365D] font-semibold text-sm"
                >
        ...
        <div>
          <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-[#D4A843] mb-3 px-1">Today</p>
          <div className="flex flex-col gap-2">
            ...
            >
              ...
              <div className="flex-1 text-left min-w-0">
                ...
                {nextAppt && (
                  <p className="text-[#B5483F] text-xs font-semibold mt-1">No driver assigned yet</p>
                )}
              </div>
              <ChevronRight size={18} color="#C4BDB3" />
            </button>
