...
export default function OnboardingPage() {
  ...
  async function share(text) {
    if (navigator.share) {
      try { await navigator.share({ title: 'Hammock365', text }) } catch { /* cancelled */ }
    } else {
  ...
  if (path === 'family') {
    ...
    return (
      <Shell step={2} total={2} onBack={() => setStep(0)}>
        <Heading title={`Invite ${name}`} sub={`${name} needs Hammock365 on their phone. The link opens straight to their button with their name on it.`} />
